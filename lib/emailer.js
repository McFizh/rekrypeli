...
function initMailer() {
    ...
    if(!smtpUname || !smtpPass) {
        ...
        console.log('>> Sendgrid SMTP username or password not set.. Disabling emailer ..');
...
async function sendResults(data) {
    ...
    let mailbody = process.env.SMTP_BODY;

    mailbody = mailbody
        .replace(/%fname%/,data.firstname)
